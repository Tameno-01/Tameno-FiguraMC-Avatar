...
function AnimClass.new(anim_name, params)
	...
	if params.synch_group ~= nil then
		if synch_groups[params.synch_group] == nil then
			synch_groups[params.synch_group] = {
				speed = 1,
				playing = {},
			}
		end
	...
	for _, group in ipairs(params.blend_groups) do
		if blend_groups[group] == nil then
			blend_groups[group] = {
				blend = 1,
				anims = {},
			}
...
function AnimClass.setGroupBlend(group, blend)
	blend_groups[group].blend = blend
	for _, anim in ipairs(blend_groups[group].anims) do
		anim:updateBlend()
	end
end
...
function AnimClass.tickStart()
	local to_delete = {}
	for anim, _ in pairs(playing_anims) do
		...
		if anim.next_blend == 0 then
			...
			if anim.synch_group ~= nil then
				synch_groups[anim.synch_group].playing[anim] = nil
			end
		end
	end
...
function AnimClass:updateBlend()
	...
	for _, group in ipairs(self.blend_groups) do
		blend = blend * blend_groups[group].blend
	end
...
function AnimClass:setActualBlend(blend)
	...
	local synch_group = synch_groups[self.synch_group]
	self.anim:setSpeed(synch_group.speed)
	local synch_group_playing_anims = synch_group.playing
	local first = Utils.isEmpty(synch_group_playing_anims)
	if not first then
		self.anim:setTime(Utils.getAnyKey(synch_group_playing_anims).anim:getTime())
	end
	synch_group_playing_anims[self] = true
end
