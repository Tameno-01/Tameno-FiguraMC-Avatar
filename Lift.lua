-- EDITED: THE WHITELIST AND ALL VELOCITY AND POSITION CONSTRAINTS ARE REMOVED
...
local proxy = setmetatable({
	...
	setRot = function(x, y, z, uuid, player)
	...
	setVel = function(x, y, z, uuid, player)
		local vec = vectors.vec3(x, y, z):applyFunc(function(v)
			return v == v and v or 0
		end)
	...
	addPos = function(x, y, z, uuid, player)
		local vec = vectors.vec3(x, y, z):applyFunc(function(v)
			return v == v and v or 0
		end)
	...
	addRot = function(x, y, z, uuid, player)
		local vec = vectors.vec2(x, y):applyFunc(function(v)
			return v == v and v or 0
		end)
	...
	addVel = function(x, y, z, uuid, player)
		local vec = vectors.vec3(x, y, z):applyFunc(function(v)
			return v == v and v or 0
		end)
	...
	forcePos = function(x, y, z, uuid, player)
		local vec = vectors.vec3(x, y, z):applyFunc(function(v)
			return v == v and v or 0
		end)
	...
	__call = function(self, uuid, usr)
		return function(key, x, y, z)
			...
			return self[key](x, y, z, uuid, player)
		end
	end,
})
...
---Creates and shares proxy function to ALL avatars.
function lib.prompter()
	-- EDITED
	for _, cur in pairs(world.getPlayers()) do
